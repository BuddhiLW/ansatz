...
(def MAX-NODES 500)
(def RECALL-LIMIT 40)
(def PRECAP 150)          ; confirm at most this many ranked candidates per goal
...
;; ---- bounded, memoized recall provider with self-exclusion ----
(defn provider-for [self-name deadline]
  (let [memo (atom {})]
    (fn [s g]
      (when (> (System/nanoTime) @deadline)
        (throw (ex-info "deadline" {::deadline true})))
      (let [gty (r/zonk s (#'r/mvar-type s g))
            k (e/->string gty)]
        (or (@memo k)
            (let [trie (deref a/ansatz-discr-trie)
                  ranked (->> ((requiring-resolve 'ansatz.tactic.discr-tree/trie-match-scored)
                               trie (recall/query-key gty))
                              (reduce (fn [m [nm sc]] (assoc m nm (max (long sc) (get m nm 0)))) {})
                              (sort-by (comp - val))
                              (remove (fn [[nm _]] (= nm self-name)))
                              (take PRECAP))
                  confirmed (->> ranked
                                 (filter (fn [[nm _]]
                                           (when (> (System/nanoTime) @deadline)
                                             (throw (ex-info "deadline" {::deadline true})))
                                           (r/applies? (:env s) (:lctx s) gty nm)))
                                 (take RECALL-LIMIT)
                                 (mapv (fn [[nm sc]] [(double (inc (long sc))) nm])))]
              (swap! memo assoc k confirmed)
              confirmed))))))
